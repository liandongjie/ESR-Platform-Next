...
def render_audit_markdown(manifest: RasterAuditManifest) -> str:
    lines: list[str] = [
        ...
        "## 1. 结论摘要",
        "",
        f"- 预期栅格：**{manifest.expected_raster_count}** 个；实际找到：**{manifest.found_raster_count}** 个。",
        f"- {manifest.expected_raster_count} 个预期文件是否齐全：**{_yes_no(manifest.all_expected_files_present)}**。",
        ...
        "### 2.1 文件数量",
        "",
        f"程序不是扫描到几个 `.tif` 就认为数据齐全，而是用项目固定的 {manifest.expected_raster_count} 项指标目录逐一匹配文件名。",
        f"因此“齐全”表示 {manifest.expected_raster_count} 个业务指标对应的文件都存在；额外的 TIFF 会单独列为 unexpected，避免把历史文件误当成指标。",
        "",
        "### 2.2 CRS 与像元对齐",
        "",
        f"对齐检查以 `{manifest.reference_file or '-'}` 作为比较基准。这个基准只用于确定性比较，没有业务优先级。",
    ...
    for record in manifest.rasters:
        ...
        if record.stats and record.stats.within_expected_range is False:
            ...
            lines.append(
                ...
                f"<0={record.stats.below_expected_min_count}，>1={record.stats.above_expected_max_count}。"
            )
            lines.append("")
